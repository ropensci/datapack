...
#'  \item{\code{\link{addRunProv}}}{: Add provenance relationships to a DataPackage for a script execution.}
#' }
...
#' @description A DataPackage can store provenance relationships between the DataObjects in the package. These
...
setGeneric("addRunProv", function(x, ...) {
    standardGeneric("addRunProv")
})

#' @rdname addRunProv
#' @param scriptId The \code{id} of the DataObject created for the script.
#' @param inIds A list of files that were read by the program
#' @param outIds A list of files that were generated by the program during the run
#' @param resolveURI The leading portion of a URL that can be used to locate objects referenced in the
...
setMethod("addRunProv", signature("DataPackage"), function(x, scriptId, inIds=list(), outIds=list(),
                                                           resolveURI=D1_CN_Resolve_URL) {
    
    # Check that pids are in the data package
    pkgIds <- getIdentifiers(x)
    if(!is.element(scriptId, pkgIds)) {
        stop(sprintf("scriptId is not a package memmber."))
    }
    if(length(inIds) > 0) {
        for (iCnt in 1:length(inIds)) {
            if (!is.element(inIds[[iCnt]], pkgIds)) {
                stop(sprintf("inId %s is not a package memmber.", inIds[[iCnt]]))
            }
        }
    }
    if(length(outIds) > 0) {
        for (iCnt in 1:length(outIds)) {
            if (!is.element(outIds[[iCnt]], pkgIds)) {
                stop(sprintf("outId %s is not a package memmber.", outIds[[iCnt]]))
            }
        }
    }
    ...
    executionIdURL <- sprintf("%s/%s", resolveURI, URLencode(executionId, reserve=T))
    ...
    if(length(outputIds) > 0 && length(inputIds > 0)) {
        for (iOutput in 1:length(outputIds)) {
