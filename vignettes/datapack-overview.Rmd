...
In that example, the `sciObj2` DataObject is added to the package usoing the `addData` call, 
and the metadata object `metadataObj` is passed in to the function as well.  This
tells the DataPackage that `metadataId cito:documents sciId2`. The `cito:documents` relationship is defined by the [Citation Typing Ontology](http://vocab.ox.ac.uk/index) (CITO)).
...
Relationships can be fully specified using the URI of the concept, as shown in the following statement that adds a provenence relationship between two objects in the example package:
...
resources. This OAI-ORE description is refered to as a *resource map*.
